...
data_t PLRUCache(
		...
		volatile data_t *dram) {
#pragma HLS INLINE off
#pragma HLS INTERFACE m_axi depth=4000 port=dram offset=direct bundle=dram
...
#pragma HLS ARRAY_PARTITION variable=tag complete dim=0
	...
	for (int i = 0; i < NUM_WAYS; i++) {
#pragma HLS UNROLL
		valid(i, i) = validArray[i_addr(INDEX_WIDTH-1, 0)](i, i);
		tag[i] = tagArray[i_addr(INDEX_WIDTH-1, 0)][i];
	}

	ap_uint<NUM_WAYS> isHit = false;
	for (int i = 0; i < NUM_WAYS; i++) {
#pragma HLS UNROLL
		isHit(i, i) = valid(i, i) && tag[i] == i_addr(INDEX_WIDTH+TAG_WIDTH-1, INDEX_WIDTH);
	}
